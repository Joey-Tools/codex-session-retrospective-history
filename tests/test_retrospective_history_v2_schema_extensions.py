...
LOGICAL_RUN_ID = canonical_run_id(1)
...
def full_manifest(
    *,
    ...
    manifest: dict[str, object] = {
        ...
        "key_id": typed_ref("key_id_v2:", width=16),
...
class RetrospectiveHistoryV2SchemaExtensionTests(unittest.TestCase):
    ...
    def test_logical_run_identity_uses_canonical_128_bit_base32(self) -> None:
        ...
        noncanonical_alias = LOGICAL_RUN_ID[:-1] + "f"
        for value in (
            "run_v2_" + "a" * 24,
            "a" * 25,
            "a" * 27,
            LOGICAL_RUN_ID.upper(),
            "0" + LOGICAL_RUN_ID[1:],
            noncanonical_alias,
            LOGICAL_RUN_ID[:13] + "/" + LOGICAL_RUN_ID[13:],
        ):
        ...
        for value in (
            "run_ref_v2:" + "a" * 32,
            "run_ref_v2:" + LOGICAL_RUN_ID.upper(),
            "run_ref_v2:" + noncanonical_alias,
        ...
        plural_keys = copy.deepcopy(manifest)
        plural_keys.pop("key_id")
        plural_keys["key_ids"] = [
            typed_ref("key_id_v2:", "a", width=16),
            typed_ref("key_id_v2:", "b", width=16),
        ]
        assert_invalid_for_both(self, plural_keys)

        mismatched_payload = copy.deepcopy(manifest)
        mismatched_payload["run_ref"] = "run_ref_v2:" + canonical_run_id(2)
