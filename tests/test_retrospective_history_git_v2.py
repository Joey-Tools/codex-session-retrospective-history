...
SECOND_RUN_ID = canonical_run_id(2)
THIRD_RUN_ID = canonical_run_id(3)
...
def run_path(
    *,
    ...
    if MODULE.RUN_ID_RE.fullmatch(run_id.encode("ascii")) is None:
        raise ValueError("run_id must be canonical 128-bit Base32")
    return Path("runs", mode, window, run_id, artifact)
...
def publication_message(path: Path) -> str:
    return publication_message_for(
        mode=path.parts[1], window=path.parts[2], run_id=path.parts[3]
    )
...
class RetrospectiveHistoryGitV2Tests(unittest.TestCase):
    ...
    def test_canonical_run_path_fits_the_git_path_budget(self) -> None:
        run_id = canonical_run_id((1 << 128) - 1)
        path = run_path(
            mode="baseline",
    ...
    def test_publisher_attestation_rejects_noncanonical_run_ref_alias(self) -> None:
        alias = RUN_ID[:-1] + "f"
        bundle_digest = "retained_bundle_digest_v2:sha256:" + "a" * 64

        with self.assertRaisesRegex(ValueError, "run_ref is invalid"):
            MODULE.publisher_attestation_payload(f"run_ref_v2:{alias}", bundle_digest)
    ...
    def test_publication_message_run_ref_must_match_the_run_path(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            base, _ = initialize_repository(root)
            artifact = write_run(root)
    ...
    def test_manifest_run_id_must_match_the_run_path(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            base, _ = initialize_repository(root)
            mismatched_manifest = (
        ...
        self.assertIn("manifest run_id does not match the physical v2 run path", issues)
    ...
    def test_old_radix_run_path_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            root = Path(raw)
            base, _ = initialize_repository(root)
            old_directory = Path(
    ...
    def test_run_path_components_must_be_canonical(self) -> None:
        canonical_parts = list(run_path().parts)
        wrong_mode_parts = canonical_parts.copy()
        wrong_mode_parts[1] = "monthly"
        wrong_window_parts = canonical_parts.copy()
        wrong_window_parts[2] = "2026-02-30"
        alias_parts = canonical_parts.copy()
        alias_parts[3] = RUN_ID[:-1] + "f"
        cases = (
            ("mode", Path(*wrong_mode_parts), publication_message_for()),
            ("window", Path(*wrong_window_parts), publication_message_for()),
            ("base32-alias", Path(*alias_parts), publication_message_for()),
        )
