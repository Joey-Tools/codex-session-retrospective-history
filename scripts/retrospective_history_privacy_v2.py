...
V2_COMMIT_MESSAGE_RE = re.compile(
    ...
    r"(?P<run_ref>run_ref_v2:[a-z2-7]{25}[aeimquy4])\n\Z"
)
...
ISSUE_BUNDLE_INVENTORY = "bundle: retained v2 privacy inventory is invalid"
ISSUE_BUNDLE_LIMIT = "bundle: retained v2 privacy scan exceeds its byte limit"
...
ARTIFACT_ORDER = tuple(sorted(ARTIFACT_BASENAMES))
...
MAX_BUNDLE_PRIVACY_BYTES = 64 * 1024 * 1024
...
def _valid_retained_path(relative: Path) -> bool:
    ...
    if any(
        ...
        return False
    root, mode, window, run_id, basename = relative.parts
    return (
        root == "runs"
        and mode in RUN_MODES
        and _valid_window(window)
        and _valid_run_id(run_id)
...
def _allowed_json_string(
    ...
    if field in {"engine_commit", "locked_first_parent_object_id"}:
        ...
        return (allowed, allowed, False)
    episode_patterns = {
        "episode_anchor": EPISODE_ANCHOR_RE,
        "episode_lineage_id": EPISODE_LINEAGE_ID_RE,
        "transition_group_id": EPISODE_TRANSITION_GROUP_ID_RE,
        "member_turn_root": EPISODE_MEMBER_TURN_ROOT_RE,
        "generalized_content_digest": EPISODE_CONTENT_DIGEST_RE,
    }
    episode_pattern = episode_patterns.get(field)
    if episode_pattern is not None:
        allowed = episode_pattern.fullmatch(value) is not None
        return (allowed, allowed, False)
    if field == "run_id":
        allowed = _valid_run_id(value)
        return (allowed, allowed, False)
...
def _scan_json_value(
    ...
    *,
    ...
    boundary_fragments: list[str] | None = None,
) -> int:
    ...
    while stack:
        ...
        if isinstance(current, dict):
            for key in sorted(current, reverse=True):
                if key not in _ALLOWED_JSON_KEYS:
                    ...
                    if boundary_fragments is not None:
                        boundary_fragments.append(key)
        ...
        if current_field == "signature" and allowed and opaque:
            ...
            continue
        if boundary_fragments is not None and not opaque:
            boundary_fragments.append(current)
...
def _scan_report(
    ...
    if boundary_fragments is not None:
        boundary_fragments.extend(body_lines)
    body = "\n".join(body_lines)
    _scan_prose_vocabulary(body, issues)


def _validate_v2_privacy(
    relative: Path,
    payload: bytes,
    boundary_fragments: list[str] | None = None,
) -> list[str]:
    ...
    if basename == "report.md":
        _scan_report(text, issues, boundary_fragments)
        return sorted(issues)
    ...
    try:
        if basename.endswith(".jsonl"):
            ...
            _scan_json_value(
                _parse_json(text), issues, boundary_fragments=boundary_fragments
            )
        else:
            issues.add(ISSUE_FORMAT)
...
def validate_v2_privacy(relative: Path, payload: bytes) -> list[str]:
    ...
    return _validate_v2_privacy(relative, payload)


def validate_v2_bundle_privacy(
    artifacts: Mapping[Path, bytes],
) -> dict[str, list[str]]:
    """Validate one complete retained bundle, including detector boundary state."""
    findings: dict[str, list[str]] = {}
    try:
        entries = [(Path(relative), payload) for relative, payload in artifacts.items()]
    except (AttributeError, TypeError, ValueError):
        return {"__bundle__": [ISSUE_BUNDLE_INVENTORY]}
    if (
        len(entries) != len(ARTIFACT_ORDER)
        or any(not isinstance(payload, bytes) for _relative, payload in entries)
    ):
        return {"__bundle__": [ISSUE_BUNDLE_INVENTORY]}
    by_basename: dict[str, tuple[Path, bytes]] = {}
    parents: set[Path] = set()
    total_bytes = 0
    for relative, payload in entries:
        if (
            relative.name not in ARTIFACT_BASENAMES
            or relative.name in by_basename
            or not _valid_retained_path(relative)
        ):
            return {"__bundle__": [ISSUE_BUNDLE_INVENTORY]}
        by_basename[relative.name] = (relative, payload)
        parents.add(relative.parent)
        total_bytes += len(payload)
        if total_bytes > MAX_BUNDLE_PRIVACY_BYTES:
            return {"__bundle__": [ISSUE_BUNDLE_LIMIT]}
    if set(by_basename) != ARTIFACT_BASENAMES or len(parents) != 1:
        return {"__bundle__": [ISSUE_BUNDLE_INVENTORY]}

    boundary_fragments: list[str] = []
    for basename in ARTIFACT_ORDER:
        relative, payload = by_basename[basename]
        issues = _validate_v2_privacy(relative, payload, boundary_fragments)
        if issues:
            findings[basename] = issues
    ...
    for fragment in boundary_fragments:
        ...
        if normalized_bytes > MAX_BUNDLE_PRIVACY_BYTES:
            findings["__bundle__"] = [ISSUE_BUNDLE_LIMIT]
            return findings
        normalized_fragments.append(normalized)
    boundary_issues: set[str] = set()
    _scan_text_risks("".join(normalized_fragments), boundary_issues)
    if boundary_issues:
        findings["__bundle__"] = sorted(boundary_issues)
    return findings
