...
V2_ADMIN_PATHS = frozenset(
    {
        ...
        b"retrospective-history-v2-admin-public.asc",
...
RUN_ID_RE = re.compile(rb"[a-z2-7]{25}[aeimquy4]")
...
COMMIT_MESSAGE_RE = re.compile(
    ...
    rb"(?P<run_ref>run_ref_v2:[a-z2-7]{25}[aeimquy4])\n"
)
...
def _valid_run_id(value: bytes) -> bool:
    if RUN_ID_RE.fullmatch(value) is None:
        return False
    try:
        decoded = base64.b32decode(value.upper() + b"======", casefold=False)
    except (binascii.Error, ValueError):
        return False
    canonical = base64.b32encode(decoded).lower().rstrip(b"=")
...
def _parse_run_path(path: bytes) -> _RunPath | None:
    ...
    if not _valid_window(window.encode("ascii")):
        return None
    if not _valid_run_id(run_id.encode("ascii")):
        return None
    ...
    for current_index, (commit_oid, parent_oid) in enumerate(commits, start=1):
        try:
            ...
            if facts.manifest_run_id != publication_run.run_id:
                issues.add(
                    f"commit {current_index}: manifest run_id does not match the physical v2 run path"
                )
